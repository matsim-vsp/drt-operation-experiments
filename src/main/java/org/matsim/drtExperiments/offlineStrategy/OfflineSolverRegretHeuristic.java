...
public class OfflineSolverRegretHeuristic implements OfflineSolver {
    private static final double NOT_FEASIBLE_COST = 1e5;  // The cost for each infeasible insertion is set to a dominantly large value (e.g., 86400)
    ...
    public FleetSchedules calculate(FleetSchedules previousSchedules, Map<Id<DvrpVehicle>, OnlineVehicleInfo> onlineVehicleInfoMap, List<GeneralRequest> newRequests, double time) {
        ...
        Map<GeneralRequest, Map<OnlineVehicleInfo, InsertionData>> insertionMatrix = new HashMap<>();
        for (GeneralRequest request : newRequests) {
            insertionMatrix.put(request, new HashMap<>());
            for (OnlineVehicleInfo vehicleInfo : onlineVehicleInfoMap.values()) {
                InsertionData insertionData = insertRequestToVehicle(vehicleInfo, request, previousSchedules, linkToLinkTravelTimeMatrix);
    ...
    record InsertionData(List<TimetableEntry> candidateTimetable, double cost, OnlineVehicleInfo vehicleInfo) {
    }

    // private methods
    private InsertionData insertRequestToVehicle(OnlineVehicleInfo vehicleInfo,
                                                 GeneralRequest request, FleetSchedules previousSchedules,
            ...
            for (int j = i + 1; j < temporaryTimetable.size() + 1; j++) {
                ...
                if (j < temporaryTimetable.size()) { // Insert drop off between two stops
                    ...
                    if (totalInsertionCost < insertionCost) {
                        ...
                        candidateTimetable = insertDropOff(temporaryTimetable, j, dropOffStopToInsert, 0); // Append at the end --> no delay to other stops
                    }
                }
            }
        }
    ...
    private InsertionData getBestInsertionForRequest(
            GeneralRequest request, Map<GeneralRequest, Map<OnlineVehicleInfo, InsertionData>> insertionMatrix) {
        double minInsertionCost = Double.MAX_VALUE;
        InsertionData bestInsertion = null;
