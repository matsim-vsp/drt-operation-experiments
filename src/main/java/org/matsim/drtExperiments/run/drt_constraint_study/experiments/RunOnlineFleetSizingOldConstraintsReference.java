...
public class RunOnlineFleetSizingOldConstraintsReference implements MATSimAppCommand {
    ...
    @CommandLine.Option(names = "--max-beta", description = "max travel time beta", defaultValue = "600")
    ...
    @CommandLine.Option(names = "--max-abs-detour", description = "max absolute delay", defaultValue = "7200")
    ...
    public static void main(String[] args) {
        new RunOnlineFleetSizingOldConstraintsReference().execute(args);
    }
    ...
    private String runSimulation(String temporaryConfig) {
        ...
        drtConfigGroup.maxDetourAlpha = 10;
        drtConfigGroup.maxDetourBeta = 7200;
        drtConfigGroup.maxAbsoluteDetour = maxAbsDetour;

        drtConfigGroup.maxWaitTime = maxWaitTime;

        drtConfigGroup.maxTravelTimeAlpha = maxTravelTimeAlpha;
        drtConfigGroup.maxTravelTimeBeta = maxTravelTimeBeta;
    ...
    private boolean analyzeResult(String outputDirectory) throws IOException {
        ...
        if (allowRejection) {
            return analysisEventsHandler.getRejectedPersons().isEmpty();
        }
